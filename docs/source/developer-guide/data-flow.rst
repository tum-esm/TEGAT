...
.. image:: ../_static/images/data-flow.png
   :alt: TEGAT data flow diagram
   :width: 80%
   :align: center


Several data flows involve TEGAT as the central component:

1. **Device Data Flow**

    Data collected by the Controller via sensors/actuators/logs is provided to TEGAT via an sqlite database.
...
2. **ThingsBoard Data Flow**
    ...
    TEGAT both receives and sends data to/from the ThingsBoard server via MQTT. Received MQTT messages include
    RPC commands, OTA Software update version information as well as file definitions/content via shared ThingsBoard
    attributes. TEGAT publishes all messages received from the controller to ThingsBoard via this MQTT channel,
...
3. **Git repository Data Flow (OTA Updates)**
    ...
    To perform over-the-air (OTA) software updates on the controller, TEGAT fetches commits in a local git
...
4. **Local file read/write data flow**
    ...
    TEGAT enables remote file management on its host device's local file system. Using ThingsBoard Shared Attributes
